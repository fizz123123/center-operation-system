...
export function getPeople(params = {}) {
  return withMockFallback(
    () => http.get('/people', { params }),
    () => mockApi.getPeople(params),
  )
}
...
export function createPerson(payload) {
  return withMockFallback(
    () => http.post('/people', payload),
    () => mockApi.createPerson(payload),
  )
}
...
export function updatePerson(id, payload) {
  return withMockFallback(
    () => http.put(`/people/${id}`, payload),
    () => mockApi.updatePerson(id, payload),
  )
}
...
export function getPersonEnrollments(personId, params = {}) {
  return withMockFallback(
    () => http.get(`/people/${personId}/enrollments`, { params }),
    () => mockApi.getPersonEnrollments(personId, params),
  )
}
